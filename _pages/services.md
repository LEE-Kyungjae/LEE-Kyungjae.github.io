...
<section class="shell service-list">
  ...
  <article class="service-row">
    ...
    <div class="service-info">
      <div class="service-title"><h2>{{ service.name_ko }}</h2><span class="status-badge"><i></i>{{ service.status_ko }}</span></div>
      <p>{{ service.one_liner_ko }}</p>
      <ul class="platform-list" aria-label="지원 플랫폼">
        {% for platform in service.platforms %}<li>{% include platform-icon.html platform=platform.id %}<span>{{ platform.label }}</span></li>{% endfor %}
      </ul>
    </div>
