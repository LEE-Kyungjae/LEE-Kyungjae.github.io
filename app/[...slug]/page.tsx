...
export default async function LegacyPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  if (permalink === '/services/' || permalink === '/en/services/') return <><Header lang={lang}/><main className="sub-page"><p className="sub-kicker">LIVE PRODUCTS</p><h1>{lang === 'ko' ? '서비스' : 'Services'}</h1><div className="service-cards">{serviceData.map(item => <a href={item[3]} key={item[1]}><i>{item[0]}</i><div><h2>{item[1]}</h2><p>{item[2]}</p></div><b>↗</b></a>)}</div></main></>;
  ...
  if (permalink === '/legal/' || permalink === '/en/legal/') {
    ...
    return <><Header lang={lang}/><main className="sub-page"><p className="sub-kicker">POLICY ARCHIVE</p><h1>{lang === 'ko' ? '운영 정책' : 'Legal'}</h1><div className="legal-list">{docs.map(doc => <a href={doc.permalink} key={doc.permalink}><span>{doc.title}</span><small>{doc.permalink}</small><b>→</b></a>)}</div></main></>;
  }

  const html = await marked.parse(entry.content);
  return <><Header lang={lang}/><main className="document"><p className="sub-kicker">DOCUMENT / {entry.file.replace(/\.(md|html)$/,'')}</p><h1>{entry.title}</h1><div className="prose" dangerouslySetInnerHTML={{ __html: html }}/><a className="back-link" href={`${prefix}/legal/`}>← {lang === 'ko' ? '목록으로' : 'Back to list'}</a></main></>;
}
